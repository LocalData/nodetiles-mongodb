...
var MongoDBSource = require('../MongoDB');


var source = new MongoDBSource({
  connectionString: process.env.DATABASE_URI || "tcp://postgres@localhost/postgis", //required
  collectionName: "rpd_parks", // required
  key: "geometry", // required
  query: {
...
describe('MongoDB Data Source', function() {
  describe('Projects between a Web Mercator (meters) Map and a Lat/Lon MongoDB data source', function() {
    /**
    ...
    // stub the MongoDB connection and client so it never fires
    var rows = [
      {
        id: 1,
        name: "Point: Spreckels Lake (Northwest)",
        geometry: [-122.494142, 37.771126] // Long, Lat!
      },
      {
        id: 2,
        name: "Point: Stow Lake (Northeast)",
        geometry: [-122.473210, 37.768930]
      },
      {
        id: 3,
        name: "Point: Devils Teeth Bakery (Southwest)",
        geometry: [-122.505069, 37.75324]
      },
      {
        id: 4,
        name: "Point: Manna Korean Restuarant(Southeast)",
        geometry: [-122.468151, 37.763694]
      },
      {
        id: 5,
